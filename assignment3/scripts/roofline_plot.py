...
for name, data in kernels.items():
    ax.plot(data['ai'], data['perf'], markers[name], color=colors[name],
            markersize=9, markeredgecolor='black', markeredgewidth=0.5,
            label=f"{name} (AI={data['ai']:.2f})", zorder=5)
...
# Peak annotations
ax.text(500, peak_flops * 1.15, f'Peak FP32: {peak_flops/1e3:.1f} TFLOPS',
        fontsize=7.5, ha='right', va='bottom', color='black')
ax.text(0.012, peak_bw * 0.012 * 1.3, f'Peak BW: {peak_bw} GB/s',
        fontsize=7.5, ha='left', va='bottom', color='black', rotation=38)
